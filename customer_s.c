...
void withdraw_money(customer* c)
{
    ...
    else
    {
        ...
        while(count != 3)
        {
            ...
            if(count == 3)
            {
                printf("Too manu unsucessful attempts, please try again later\n");
            }
        }
    }
}
...
void make_deposit(customer* c)
{
    ...
    int count = 0;
    char *temp, temp1[20];
    temp = decrypt(c ->upi_passcode, temp1);
    while(count != 3)
    {
        ...
        if(count == 3)
        {
            printf("To many unsuccessful attempts, please try again later\n");
        }
    }  
}
